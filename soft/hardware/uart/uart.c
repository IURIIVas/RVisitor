...
void uart_init(USART_TypeDef *uart_x, uart_init_s *uart_init_struct)
{
    ...
    if(uart_init_struct->USART_HardwareFlowControl != USART_HardwareFlowControl_None)
    {
    }

    usartxbase = (uint32_t)uart_x;
    tmpreg = uart_x->CTLR2;
    tmpreg &= CTLR2_STOP_CLEAR_Mask;
    tmpreg |= (uint32_t)uart_init_struct->USART_StopBits;
    uart_x->CTLR2 = (uint16_t)tmpreg;

    tmpreg = uart_x->CTLR1;
    tmpreg &= CTLR1_CLEAR_Mask;
    tmpreg |= (uint32_t)uart_init_struct->USART_WordLength
    		| uart_init_struct->USART_Parity
			| uart_init_struct->USART_Mode;
    uart_x->CTLR1 = (uint16_t)tmpreg;

    tmpreg = uart_x->CTLR3;
    tmpreg &= CTLR3_CLEAR_Mask;
    tmpreg |= uart_init_struct->USART_HardwareFlowControl;
    uart_x->CTLR3 = (uint16_t)tmpreg;
    ...
    if((uart_x->CTLR1 & CTLR1_OVER8_Set) != 0)
    {
        integerdivider = ((25 * apbclock) / (2 * (uart_init_struct->USART_BaudRate)));
    }
    else
    {
        integerdivider = ((25 * apbclock) / (4 * (uart_init_struct->USART_BaudRate)));
    }
...
void uart_send_str(USART_TypeDef *USARTx, const char *str_to_send)
{
	while (*str_to_send)
	{
		uint8_t tmp_char = (uint8_t)(*str_to_send);
