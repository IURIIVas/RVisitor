...
static uint8_t _spi_flash_wait_txen(spi_s *spi)
{
    uint8_t timeout = 200;

    while (SPI_I2S_GetFlagStatus(spi, SPI_I2S_FLAG_TXE) == RESET && timeout)
    {
        timeout--;
    }
    return timeout;
...
static uint8_t _spi_flash_wait_rxne(spi_s *spi)
{
    uint8_t timeout = 200;

    while (SPI_I2S_GetFlagStatus(spi, SPI_I2S_FLAG_RXNE) == RESET && timeout)
    {
        timeout--;
    }
    return timeout;
 ...
 * @return  none
 */
void SPI_I2S_DeInit(spi_s *SPIx)
{
    if(SPIx == SPI1)
    {
 ...
 * @return  none
 */
void SPI_Init(spi_s *SPIx,  spi_init_s *SPI_InitStruct)
{
    uint16_t tmpreg = 0;
 ...
 * @return  none
 */
void I2S_Init(spi_s *SPIx, I2S_InitTypeDef *I2S_InitStruct)
{
 ...
 * @return  none
 */
void SPI_StructInit( spi_init_s *SPI_InitStruct)
{
 ...
 * @return  none
 */
void SPI_I2S_SendData(spi_s *SPIx, uint16_t Data)
{
    SPIx->DATAR = Data;
}
 ...
 */
uint16_t SPI_I2S_ReceiveData(spi_s *SPIx)
{
    return SPIx->DATAR;
}
 ...
 * @return  none
 */
void SPI_NSSInternalSoftwareConfig(spi_s *SPIx, uint16_t SPI_NSSInternalSoft)
{
 ...
 * @return  none
 */
void SPI_DataSizeConfig(spi_s *SPIx, uint16_t SPI_DataSize)
{
 ...
 * @return  none
 */
void SPI_TransmitCRC(spi_s *SPIx)
{
 ...
 */
uint16_t SPI_GetCRC(spi_s *SPIx, uint8_t SPI_CRC)
{
    uint16_t crcreg = 0;
 ...
 */
uint16_t SPI_GetCRCPolynomial(spi_s *SPIx)
{
    return SPIx->CRCR;
}
 ...
 * @return  none
 */
void SPI_BiDirectionalLineConfig(spi_s *SPIx, uint16_t SPI_Direction)
{
 ...
 * @return  FlagStatus: SET or RESET.
 */
void SPI_I2S_ClearFlag(spi_s *SPIx, uint16_t SPI_I2S_FLAG)
{
 ...
 * @return  none
 */
void SPI_I2S_ClearITPendingBit(spi_s *SPIx, uint8_t SPI_I2S_IT)
{
    uint16_t itpos = 0;
...
uint8_t spi_readwrite_byte(spi_s *spi, uint8_t tx_data)
{
    uint8_t status = 0;
    status = _spi_flash_wait_txen(spi);
    if (!status)
    {
        return status;
    }
    ...
    status = _spi_flash_wait_rxne(spi);
    if (!status)
    {
        return status;
    }
