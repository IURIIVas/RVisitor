 ...
 * @return  none
 */
uint32_t I2C_GetLastEvent(i2c_s *I2Cx)
{
    uint32_t lastevent = 0;
    uint32_t flag1 = 0, flag2 = 0;
...
{
    while ((I2C_GetFlagStatus(I2Cx, I2C_FLAG_BUSY) != RESET) && timeout)
    {
        timeout--;
        if (0 == timeout)
        {
    ...
    while((I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_MODE_SELECT) != READY) && timeout)
    {
        timeout--;
        if (0 == timeout)
        {
    ...
    while((I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED) != READY) && timeout)
    {
        timeout--;
        if (0 == timeout)
        {
    ...
    while((I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED) != READY) && timeout)
    {
        timeout--;
        if (0 == timeout)
        {
...
status_e i2c_master_receive(i2c_s *I2Cx, uint8_t dev_addr, uint8_t *receive, uint16_t size, uint32_t timeout)
{
    ...
    while((I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_MODE_SELECT) != RESET) && timeout)
    {
        timeout--;
        if (0 == timeout)
        {
    ...
    while((I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED) != RESET) && timeout)
    {
        timeout--;
        if (0 == timeout)
        {
    ...
    while((I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED) != RESET) && timeout)
    {
        timeout--;
        if (0 == timeout)
        {
    ...
    while((I2C_GetFlagStatus(I2Cx, I2C_FLAG_RXNE) == RESET) && timeout)
    {
    ...
    for (uint32_t i = 0; i < size; i++)
    {
        receive[i] = I2C_ReceiveData(I2Cx);
    }
